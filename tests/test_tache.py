...
def test_to_dict_from_dict():
    ...
    assert tache2.get_description() == "Test description", "Erreur sur la description reconstruit"
    assert tache2.get_priorite() == 5, "Erreur sur la priorité reconstruit"
    assert tache2.get_date_limite() == "2025-01-01", "Erreur sur la date limite reconstruit"
...
def test_str_representation():
    ...
    assert "Titre: Faire les courses" in rep, "La représentation en chaîne ne contient pas le titre"
...
def test_generate_unique_id():
    ...
    assert new_id not in {111111, 222222}, "L'ID généré doit être unique par rapport aux tâches existantes"
    assert 100000 <= new_id <= 999999, "L'ID doit être un nombre à 6 chiffres"

    # Test de génération avec une liste vide
    tasks = []
    new_id = generate_unique_id(tasks)
    assert 100000 <= new_id <= 999999, "L'ID doit être un nombre à 6 chiffres pour une liste vide"
