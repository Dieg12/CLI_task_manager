...
def main():
    ...
    parser.add_argument("--version", action="store_true", help="Affiche la version de l'application.")
    
    # Création des sous-commandes
    subparsers = parser.add_subparsers(dest="command", title="Commandes disponibles", help="Choisissez une commande à exécuter")
    ...
    parser_add.add_argument("--priority", type=int, default=1, help="Priorité de la tâche (défaut: 1)")
    parser_add.add_argument("--due", help="Date d'échéance de la tâche (format YYYY-MM-DD)")
    ...
    parser_remove.add_argument("--id", required=True, help="Identifiant de la tâche à supprimer")
    
    # Commande 'liste' : Affiche la liste des tâches
    parser_list = subparsers.add_parser("list", help="Affiche la liste des tâches")
    parser_list.add_argument("--sort", choices=["title", "priority", "due"],
                                help=("Trier la liste par :\n"
